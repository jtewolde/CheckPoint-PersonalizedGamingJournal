...
export default function GameCard({
  ...
  const getPlatformIcon = (platform: string) => {
    ...
    // Apple
    if (name.includes("mac")) return <FaApple size={18} />;

    // Google Stadia
    if (name.includes("google stadia")) return <FaGoogle size={18} />;

    // Linux
    if (name.includes("linux")) return <FaLinux size={18} />;

    // Steam
    if (name.includes("steam")) return <FaSteam size={18} />;

    // Android
    if (name.includes("android")) return <FaAndroid size={18} />;

    // iOS
    if (name.includes("ios")) return <FaApple size={18} />;
    ...
    if (name.includes("playstation")) return <SiPlaystation size={30} />;
  ...
  return (
    <div
      key={game.id}
      className={`${classes.gameCard} ${variant === "compact" ? classes.compact : variant === "small" ? classes.small : variant === "library" ? classes.library : classes.default}`}
      onClick={() => {
          ...
          <>
              ...
              <Badge
                ...
                color={
                  game.total_rating && game.total_rating >= 80
                    ? "#1ace3b"
                    : game.total_rating && game.total_rating >= 70
                      ? "yellow"
                      : "#f01e1e"
                }
                radius="md"
            ...
            {showGameInfo && game.game_type?.type !== "Main Game" && (
              <Badge
                ...
                radius="sm"
                color={
                  game.game_type?.type == "Expansion"
                    ? "grape"
                    : game.game_type?.type == "Remake"
                      ? "blue"
                      : "#2e2e2e"
                }
              >
        ...
        {variant === "library" && (
          <>
            ...
            {libraryMeta?.platinum === true && (
              <ThemeIcon
                className={classes.platinumIcon}
                size="md"
                radius="sm"
                variant="outline"
                color="black"
              >
                <FaTrophy size={20} color="gold" cursor="pointer" />
              </ThemeIcon>
            )}
          </>
        )}
        ...
        <div className={classes.gameInfo}>
          ...
          <Group gap={8}>
            {visiblePlatforms.map((platform) => (
              <Tooltip key={platform.name} label={platform.name} withArrow>
