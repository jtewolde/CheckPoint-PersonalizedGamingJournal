...
export async function GET(req: NextRequest) {
  try {
    ...
    const cacheKey = `igdb_trending_games:
    ${limit}:
    ${offset}:
    ${sort || ""}:
    ${game_types.join(",")}:
    ...
    const gamesRes = await fetch("https://api.igdb.com/v4/games", {
      ...
      body: `fields name, summary, genres, genres.name, genres.slug, cover.url, version_title, game_type.type, game_modes.slug, game_modes.name, themes.slug, platforms.name, platforms.abbreviation, platforms.slug, first_release_date, release_dates.human, total_rating; where ${whereClause}; ${sortClause} limit ${limit}; offset ${offset};`,
    });
