...
export async function GET(req: NextRequest) {
  try {
    ...
    const body = searchQuery
      ? `fields name, summary, genres, genres.slug, cover.url, version_title, game_type.type, themes.slug, platforms.name, first_release_date, total_rating; where version_parent = null & total_rating >= 30 & name ~ *"${searchQuery}"*; sort total_rating desc; limit ${limit}; offset ${offset};`
