  ...
  try {
    ...
    if (searchQuery) {
      ...
      whereConditions.push(
        `first_release_date <= ${Math.floor(now.getTime() / 1000)}`,
      );
    ...
    if (game_types.length) {
      whereConditions.push(
        `game_type.type = (${game_types.map((t) => `"${t}"`).join(",")})`,
      );
    }
    if (genres.length) {
      whereConditions.push(
        `genres.slug = (${genres.map((g) => `"${g}"`).join(",")})`,
      );
    }
    if (themes.length) {
      whereConditions.push(
        `themes.slug = (${themes.map((t) => `"${t}"`).join(",")})`,
      );
    }
    if (modes.length) {
      whereConditions.push(
        `game_modes.slug = (${modes.map((m) => `"${m}"`).join(",")})`,
      );
    }
    if (platforms.length) {
      whereConditions.push(
        `platforms.slug = (${platforms.map((p) => `"${p}"`).join(",")})`,
      );
    }
    ...
    const body = searchQuery
      ? `fields name, summary, genres.name, genres.slug, cover.url, version_title, game_type.type, game_modes.slug, game_modes.name, screenshots.url, themes.slug, platforms.name, platforms.alternative_name, platforms.slug, platforms.abbreviation, platforms.platform_logo.url, first_release_date, release_dates.human, total_rating; where ${whereClause}; ${sortClause} limit ${limit}; offset ${offset};`
    ...
    if (!countRes.ok) {
      ...
      return NextResponse.json(
        {
          message: "Failed to fetch the count of games from IGDB",
          error: errorData,
        },
