...
export async function GET(req: NextRequest) {
  ...
  // Add filters to the search query for IGDB games API request if there are any applied to the whereConditions
    ...
    const gamesRes = await fetch('https://api.igdb.com/v4/games', {
      ...
      body: `fields name, summary, genres, hypes, genres.name, genres.slug, cover.url, version_title, game_type.type, game_modes.slug, themes.slug, platforms.name, platforms.abbreviation, platforms.slug, first_release_date, release_dates.human, total_rating; where ${whereClause}; ${sortClause} limit ${limit}; offset ${offset};`
    });
