...
export default function UpcomingPage() {
  ...
  const [sortOption, setSortOption] = useState<
    ...
    | ""
  >("first_release_date_oldest"); // State to sort search results from release date/total_rating
    ...
    const fetchUpcomingGames = async () => {
      try {
        ...
        const res = await fetch(
          `/api/igdb/upcoming-releases?${params.toString()}`,
        );
        ...
        <div className={classes.headerSection}>
          <div className={classes.titleDescriptionSection}>
            <div className={classes.titleLogo}>
              <h1 className={classes.titleText}>Upcoming Games</h1>
            </div>

            <Text className={classes.description}>
              Discover the latest games that are going to the market soon!
            </Text>
          </div>
