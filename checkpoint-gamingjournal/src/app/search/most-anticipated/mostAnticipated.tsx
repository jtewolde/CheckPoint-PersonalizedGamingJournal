...
import classes from './mostAnticipated.module.css';
...
export default function MostAnticipated() {
  ...
  const [sortOption, setSortOption] = useState<'first_release_date' | 'total_rating' | 'alphabetical' | 'first_release_date_oldest' | 'hypes' | 'hypes_reverse' | ''>('hypes'); // State to sort search results from release date/total_rating
  ...
  useEffect(() => {
    const fetchMostAnticipated = async () => {
        try {
            ...
            const res = await fetch(`/api/igdb/most-anticipated?${params.toString()}`);
          ...
          <div className={classes.headerSection}>
            <div className={classes.titleDescriptionSection}>
              ...
              <Text className={classes.description}>
                  Browse the highest-hyped releases and never miss the next big launch.
              </Text>
            </div>
                  ...
                  {/* Sort By Dropdown */}
                    <Select
                      ...
                      data={[
                          ...
                          { value: 'hypes', label: 'Hypes (Most)' },
                          { value: 'hypes_reverse', label: 'Hypes (Least)' },
                      ]}
                      value={sortOption}
            ...
            {totalPages > 1 && (
              <div className={classes.paginationWrapper}>
                  <Pagination
                      ...
                      onChange={(newPage) => {
                          setPage(newPage);
                          router.push(`/search/most-anticipated?&page=${newPage}`);
                      }}
                  />
              </div>
            )}
